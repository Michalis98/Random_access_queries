...
public class BTree <Key extends Comparable<Key>>{
	...
	private class Entry <Key extends Comparable<Key> > implements Comparable<Entry>{
	...
	********************************************************/
	...
	public int get(Comparable key){
		level=0;
		int found = search(root, key, HT);
		return found;
	}
	private int search(Node x, Comparable key, int ht){
		if (x == null)
			return level;
	...
	public void insert(Comparable key){
		Node newRoot = insert(root, key, HT);
		N++;
		if(newRoot == null) return;
		root = newRoot;
		HT++;
	}
	private Node insert(Node curr, Comparable key, int ht){
		int indexToInsert;
		if(ht == 0){//leaf nodes level
			...
			curr.add(new Entry(key), indexToInsert);
			...
			Node u = insert(curr.children[indexToInsert], key, ht - 1);
